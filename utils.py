...
def refresh_brawl_stars_api_token_if_enabled(config, file_path="cfg/brawl_stars_api.toml", force=False):
    ...
    key_name_prefix = str(config.get("key_name_prefix", "Amethyst Auto")).strip() or "Amethyst Auto"
    ...
    description = str(config.get("key_description", "Auto-generated by Amethyst for the current public IP."))
    create_payload = {
        "name": key_name,
...
async def async_notify_user(message_type: str | None = None, screenshot: Image = None, details=None) -> bool:
    ...
    discord_ok = await send_discord_notification(message_type, screenshot=screenshot, details=details)
    telegram_ok = await send_telegram_notification(message_type, screenshot=screenshot, details=details)
