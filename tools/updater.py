...
def download_file(url: str, destination: Path, label: str) -> Path:
    print(f"Downloading latest Amethyst update ({label})...")
    request = urllib.request.Request(url, headers={"User-Agent": "Amethyst-Updater"})
...
def find_project_root(extracted_dir: Path) -> Path:
    ...
    if not candidates:
        raise FileNotFoundError("Downloaded update does not look like a Amethyst project.")
...
def main() -> int:
    ...
    if not (project_dir / "main.py").exists():
        print("updater.exe must be inside the Amethyst project folder next to main.py.")
        wait_for_enter()
        return 1

    if "--smoke-test" in sys.argv:
        print("Smoke test passed. Updater can see the Amethyst project folder.")
        return 0
