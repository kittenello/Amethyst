...
def load_webhook_settings() -> dict[str, Any]:
    ...
    webhook_config.setdefault("username", "Amethyst")
...
def _title_and_description(event_type: str, details: dict[str, Any]) -> tuple[str, str]:
    ...
    if event_type == "completed":
        return "🏆 All Targets Complete", "Amethyst finished every queued target."
    if event_type == "bot_is_stuck":
        reason = str(details.get("reason") or "Amethyst could not recover automatically.")
        return "🚨 Bot Needs Attention", reason
    if event_type == "test":
        return "🧪 Webhook Test", "Discord webhook is connected correctly."
    return "📣 Amethyst Update", str(details.get("message") or "Bot event received.")
...
async def async_notify_user(
    ...
    embed.set_footer(text="Amethyst")
    ...
    send_kwargs = {
        "embed": embed,
        "username": str(settings.get("username") or "Amethyst"),
...
async def async_send_test_notification() -> bool:
    return await async_notify_user(
        ...
        details={
            "state": "configured",
            "message": "This is a manual test from the Amethyst Hub.",
        },
    )
