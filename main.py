...
def pyla_main(data):
    ...
    class Main:
        ...
        def __init__(self):
            ...
            if first_row.get('automatically_pick'):
...
def run_app():
    ...
    while True:
        if selected_data:
            ...
            utils.save_brawler_data(selected_data)
            time.sleep(0.05)
            pyla_main(selected_data)

        selected_data = []
        web_app.selected_data = None
        web_app.ready.clear()
        print("Amethyst webapp is waiting for START from the dashboard...")
        web_app.ready.wait()
        selected_data = web_app.selected_data or []
